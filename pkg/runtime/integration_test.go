...
forissue:
	...
	if err != nil {
		t.Fatalf("unable to create topic %q: %v", "orders.created", err)
	}
	...
	if result.SchemaID != registered.ID {
		t.Fatalf("schema ID = %d, want %d", result.SchemaID, registered.ID)
	}
	for {
